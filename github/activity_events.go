...
func (s *ActivityService) ListEvents(ctx context.Context, opt *ListOptions) ([]*Event, *Response, error) {
	return s.ListEventsWithETag(ctx, opt, "")
}

// ListEventsWithETag drinks from the firehose of all public events across GitHub from the last ETag.
//
// GitHub API docs: https://developer.github.com/v3/activity/events/#list-public-events
func (s *ActivityService) ListEventsWithETag(ctx context.Context, opt *ListOptions, etag string) ([]*Event, *Response, error) {
	...
	req, err := s.client.NewRequestWithHeader("GET", u, nil, header)
	if err != nil {
		return nil, nil, err
	}
